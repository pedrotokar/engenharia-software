...
interfaces desses produtos, e existem produtos concretos derivados de 
diferentes produtos abstratos que tem algo em comum entre si, e devem ser
usados em conjunto. Por exemplo, podem existir produtos abstratos como
componentes de inteface gráfica (scroll, botão, etc) e implementações desses 
produtos abstratos para diferentes bibliotecas de desenho (Qt, GTK).
...
produto abstrato, já que todas as factorys existentes teriam que ser
modificadas para adicionar um novo método para esse produto novo. É possível
contornar isso fazendo um método de criação de produtos que receba um
parâmetro, no lugar de vários métodos. Mas isso pode causar inconsistências
entre os possíveis parâmetros que são aceitos por cada factory específica.
